...
namespace Common
{
    public class TraceListenerTextbox : TraceListener, ISupportFilters
    {
        #region const
        ...
        public const string CONFIGSETTING_TRACEMESSAGEFORMAT = "TraceMessageFormat"; public const string CONFIGDEFAULT_TRACEMESSAGEFORMAT = "[{now}] {source} {category} {tidpid} - {sourceLevel} - {nesting}   {message}";
        ...
        public const string CONFIGSETTING_TRACEMESSAGEFORMATSTART = "TraceMessageFormatStart"; public const string CONFIGDEFAULT_TRACEMESSAGEFORMATSTART = "[{now}] {source} {category} {tidpid} - {sourceLevel} - {nesting} {message}";
        public const string CONFIGSETTING_TRACEMESSAGEFORMATSTOP = "TraceMessageFormatStop"; public const string CONFIGDEFAULT_TRACEMESSAGEFORMATSTOP = "[{now}] {source} {category} {tidpid} - {sourceLevel} - {nesting} {message} ({delta} secs)";
        public const string CONFIGSETTING_TRACEMESSAGEFORMATINLINESTOP = "TraceMessageFormatInlineStop"; public const string CONFIGDEFAULT_TRACEMESSAGEFORMATINLINESTOP = "... END ({delta} secs)";
        ...
        #region .ctor
        public TraceListenerTextbox() : base() { Init(); }
        public TraceListenerTextbox(TextBox txtOut) : this() { TextBox = txtOut; }
        public TraceListenerTextbox(TextBox txtOut, string cRReplace, string lFReplace, string timestampFormat, bool? showNestedFlow, bool? flushOnWrite, int? processNamePadding, int? sourcePadding, int? categoryPadding, int? sourceLevelPadding,
                                                    string traceMessageFormat, string traceMessageFormatVerbose, string traceMessageFormatInformation, string traceMessageFormatWarning, string traceMessageFormatError, string traceMessageFormatCritical,
                                                    ...
                                                    string filter, string categoryFilter, TraceEventType? allowedEventTypes) // : this(txtOut)
        {
            Init(txtOut, cRReplace, lFReplace, timestampFormat, showNestedFlow, flushOnWrite, processNamePadding, sourcePadding, categoryPadding, sourceLevelPadding,
                 traceMessageFormat, traceMessageFormatVerbose, traceMessageFormatInformation, traceMessageFormatWarning, traceMessageFormatError, traceMessageFormatCritical,
                 traceMessageFormatStart, traceMessageFormatStop, traceMessageFormatSuspend, traceMessageFormatResume, traceMessageFormatTransfer,
                 filter, categoryFilter, allowedEventTypes);
        }
        private void Init()
        {
        ...
        private void Init(TextBox txtOut, string cRReplace, string lFReplace, string timestampFormat, bool? showNestedFlow, bool? flushOnWrite, int? processNamePadding, int? sourcePadding, int? categoryPadding, int? sourceLevelPadding,
                                                    string traceMessageFormat, string traceMessageFormatVerbose, string traceMessageFormatInformation, string traceMessageFormatWarning, string traceMessageFormatError, string traceMessageFormatCritical,
                                                    string traceMessageFormatStart, string traceMessageFormatStop, string traceMessageFormatSuspend, string traceMessageFormatResume, string traceMessageFormatTransfer,
                                                    string filter, string categoryFilter, TraceEventType? allowedEventTypes)
        {
            ...
            if (allowedEventTypes != null) { _allowedEventTypes = allowedEventTypes.Value; }
        ...
        public override void Write(object o)
        {
            ...
            TextBox.Dispatcher.Invoke(new AppendTextDelegate((s) =>
            {
                ...
                {
                    var lines2Drop = TextBox.LineCount + entries.Count - MINLINES;
                    var text = TextBox.Text + sbMessages.ToString();
                    var index = text.IndexOfNth(Environment.NewLine, lines2Drop);
                    var truncatedText = index >= 0 ? text.Substring(index) : "";
                    TextBox.Text = truncatedText; TextBox.ScrollToEnd();
                    return;
                }
                if (sbMessages.Length > 0) { TextBox.AppendText(sbMessages.ToString()); TextBox.ScrollToEnd(); }
            }), new object[] { null });
        }
